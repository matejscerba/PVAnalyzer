...
protected:
     ...
     * @note This constructor can be called by derived structs only.
     */
    single_value_parameter(const std::string name, const std::string &&unit) noexcept
        : parameter(name, vault_part::all, std::move(unit)) {}
...
protected:
     ...
     * @note This constructor can be called by derived structs only.
     */
    takeoff_parameter(std::size_t takeoff, const std::string name, const std::string &&unit, double fps) noexcept
        : single_value_parameter(name, std::move(unit)), takeoff(takeoff), frames(std::max((int)(TAKEOFF_PARAM_TIME * fps), 1)) {}
...
public:
     ...
     * @param name Name of this parameter.
     * @param a, b Body parts whose mean velocity loss should be computed.
     */
    velocity_loss(std::size_t takeoff, const std::string name, body_part a, body_part b, double fps) noexcept
        : takeoff_parameter(takeoff, name, " %", fps), a(a), b(b) {}
    ...
    virtual void compute(const model_video_points &points) noexcept {
        if ((takeoff >= frames) && (points.size() > takeoff + frames)) {
            model_points before = points[takeoff - frames];
            model_points during = points[takeoff];
            model_points after = points[takeoff + frames];
            ...
            if (p_before && p_during && p_after && p_during->x - p_before->x != 0) {
...
public:
     ...
     * @param takeoff Number of frame in which athlete takes off.
     */
    hips_velocity_loss(std::size_t takeoff, double fps) noexcept
        : velocity_loss(takeoff, "Hips velocity loss", body_part::l_hip, body_part::r_hip, fps) {}
...
public:
     ...
     * @param takeoff Number of frame in which athlete takes off.
     */
    shoulders_velocity_loss(std::size_t takeoff, double fps) noexcept
        : velocity_loss(takeoff, "Shoulders velocity loss", body_part::l_shoulder, body_part::r_shoulder, fps) {}
...
public:
     ...
     * @param takeoff Number of frame in which athlete takes off.
     */
    takeoff_angle(std::size_t takeoff, double fps) noexcept
        : takeoff_parameter(takeoff, "Takeoff angle", "°", fps) {}
    ...
    virtual void compute(const model_video_points &points) noexcept {
        if ((takeoff >= frames) && (points.size() > takeoff + frames)) {
            ...
            model_points during = points[takeoff];
            model_points after = points[takeoff + frames];
            model_point p_before = (before[body_part::l_hip] + before[body_part::r_hip]) / 2.0;
            ...
            if (p_before && p_during && p_after) {
                double runup_angle = get_horizontal_tilt_angle(*p_before, *p_during);
                double takeoff_angle = get_horizontal_tilt_angle(*p_during, *p_after);
                value = takeoff_angle - runup_angle;
            } else {
                value = std::nullopt;
            }
        } else {
...
protected:
     ...
     * @note This constructor can be called by derived structs only.
     */
    steps_parameter(const std::string name, const vault_part part, const std::string &&unit, double fps) noexcept
        : multiple_values_parameter(name, vault_part::runup, std::move(unit)), fps(fps) {}
...
public:
    ...
    virtual void compute(const model_video_points &points) noexcept {
        values.clear();
        step_frames = get_step_frames(points, fps);
    ...
    virtual void write_value(std::ostream &os, std::size_t frame_no, bool write_unit) const noexcept {
        std::size_t after = 0;
        for (; after < step_frames.size(); ++after) {
            if (step_frames[after] >= frame_no) {
                // `frame_no` is before `step_frames[after]`.
                break;
            }
        }
        if (after && write_unit) {
            os << "Step number " << after << " ";
        }
        if (after && (frame_no <= step_frames.back())) {
            os << *values[after - 1] << (write_unit ? unit : "");
        } else {
            os << "?";
        }
    }
...
public:
     ...
     * @param fps Frame rate of processed video.
     */
    steps_angle(double fps) noexcept
        : steps_parameter("Step angle", vault_part::runup, "°", fps) {}
     ...
     */
    virtual void compute(const model_video_points &points) noexcept {
        std::vector steps = get_step_frames(points, fps);
        model_video_points reversed(points.rbegin(), points.rend());
        std::vector<std::size_t> step_beg_frames = get_frame_numbers(reversed.begin(), reversed.end(), std::less<double>(), fps);
    ...
    virtual void write_value(std::ostream &os, std::size_t frame_no, bool write_unit) const noexcept {
        ...
        if (found != step_frames.end()) {
            if (write_unit) {
                os << "Step number " << found - step_frames.begin() + 1 << " ";
            }
...
private:
    ...
    virtual std::optional<double> extract_value(const model_points &body) const noexcept {
        return get_vertical_tilt_angle(
            (body[body_part::l_hip] + body[body_part::r_hip]) / 2.0,
            get_part(body[body_part::l_ankle], body[body_part::r_ankle], std::less<double>()),
            tilt_angle_side::bottom
        );
    }
};
...
protected:
     ...
     * @note This constructor can be called by derived structs only.
     */
    frame_wise_parameter(const std::string name, const std::string &&unit) noexcept
        : multiple_values_parameter(name, vault_part::all, std::move(unit)) {}
...
public:
    ...
    hips_height() noexcept
        : frame_wise_parameter("Hips height", " px") {}
...
public:
    ...
    body_part_height(body_part b_part) noexcept
        : frame_wise_parameter(body_part_name(b_part) + " height", " px"), b_part(b_part) {}
...
public:
     ...
     */
    vertical_tilt(std::string name, body_part a1, body_part a2, body_part b1, body_part b2) noexcept
        : frame_wise_parameter(name, "°"), a1_part(a1), a2_part(a2), b1_part(b1), b2_part(b2) {}
...
protected:
     ...
     */
    virtual std::optional<double> extract_value(const model_points &body) const noexcept {
        return get_vertical_tilt_angle((body[a1_part] + body[a2_part]) / 2.0, (body[b1_part] + body[b2_part]) / 2.0, tilt_angle_side::top);
    }
